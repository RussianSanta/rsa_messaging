...
class ServerThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            out.flush();
            System.out.println("//Отправлен открытый ключ сервера//");
            while (true) {
                word = in.readLine();
                System.out.println("Полученное сообщение: " + word);
                if(word.equals("/stop")) {
                    System.out.println("Клиент отключен. Адрес: " + socket.getInetAddress());
                    break;                }
